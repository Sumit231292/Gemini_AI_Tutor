...
│              │                               │                  │
└──────────────┼───────────────────────────────┼─────────────────┘
               │ Bidirectional Audio           │ User Auth &
               │ Streaming                     │ Profile Storage
               ▼                               ▼
┌──────────────────────────────┐  ┌────────────────────────────┐
│      Google AI Platform      │  │  Google Cloud Firestore    │
│  ┌──────────────────────┐    │  │                            │
│  │  Gemini 2.5 Flash    │    │  │  • User profiles           │
│  │  Native Audio        │    │  │  • Username / password     │
│  │  (Live API — voice)  │    │  │  • Grade, age, gender      │
│  │  • Real-time audio   │    │  │  • Language preference     │
│  │  • Interruptions     │    │  │  • Subject preference      │
│  │  • Transcription     │    │  │                            │
│  └──────────────────────┘    │  │  (Fallback: local JSON)    │
│  ┌──────────────────────┐    │  └────────────────────────────┘
│  │  Gemini 2.5 Flash    │    │
...
│   │   └── user_store.py        # User auth & Firestore/JSON storage
│   ├── data/
│   │   └── users.json           # Local fallback user storage
│   ├── requirements.txt         # Python dependencies
│   └── Dockerfile               # Container for Cloud Run
├── frontend/
│   ├── index.html               # Main UI (sign-in, landing, session)
